...
public class ResearcherController {
	...
	public void inserirPesquisador(Research research, Address address,
		...
		/* Campos obrigatórios de pesquisador nacional */

		if (research.isEstrangeiro() == false) {
			if ((research.getNome() == null)
					|| (verificadorEmail.isEmail(validaEmail) == false)
					|| (research.getSenha() == null)
					|| (etniaId == null)
					|| (areaId == null)
					|| (verificadorCpf.isCpf(validaCpf) == false)
					|| (verificadorRg.isRg(validaRg) == false)
					|| (verificadorTelefone.isTelefone(validaTelefone) == false)) {
				com.sigfap.admin.json.research.Error error1 = new com.sigfap.admin.json.research.Error(
						"Preencher nome, e-mail, senha, CPF, RG, telefone, etnia ou área de conhecimento corretamente.");
				result1.use(Results.json()).from(error1).recursive()
						.serialize();
		...
		/* Campos obrigatórios de pesquisador estrangeiro */

		else if ((research.getNome() == null)
				|| (verificadorEmail.isEmail(validaEmail) == false)
				|| (research.getSenha() == null) || (etniaId == null)
				|| (areaId == null)
				|| (verificadorTelefone.isTelefone(validaTelefone) == false)
				|| (address.getCidadeEstrangeira() == null)
				|| (address2.getCidadeEstrangeira() == null)) {
			com.sigfap.admin.json.research.Error error2 = new com.sigfap.admin.json.research.Error(
					"Preencher nome, e-mail, senha, cidade estrangeira residencial, cidade estrangeira comercial, telefone, etnia ou área de conhecimento corretamente.");
	...
	@Public
	@Put("/v1/researcher")
	public void editarPesquisador(int id, Research research) {
		
		String validaEmail2 = research.getEmail();
		
		if (verificadorEmail.isEmail(validaEmail2) == false) {
			com.sigfap.admin.json.research.Error error = new com.sigfap.admin.json.research.Error("E-mail inválido.");
			result1.use(Results.json()).from(error).serialize();
			result1.include(error);
			return;			
		}
		...
		if (research.getSenha() != null) {
			...
			try {
				...
				com.sigfap.admin.json.research.Error error2 = new com.sigfap.admin.json.research.Error("Impossível editar pesquisador.");
